...
class MetricList(EvalMetric):
	...
	def update(self, preds, labels, losses=None, clip_loss=None):
		...
		clip_loss = [clip_loss] if type(clip_loss) is not list else clip_loss

		for metric in self.metrics:
			metric.update(preds, labels, losses, clip_loss)
...
class CLIP_Loss(EvalMetric):
	...
	def __init__(self, max_rul, name='CLIP_Loss'):
		super(CLIP_Loss, self).__init__(name)
		self.max_rul = max_rul

	def update(self, preds, labels, losses, clip_loss):
		assert losses is not None, "Loss undefined."

		for loss in clip_loss:
			self.sum_metric += float(loss.numpy().sum())*labels[0].shape[0]
			self.num_inst += labels[0].shape[0]
