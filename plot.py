...
val_keys = ['Test_RMSE', 'Test_RULscore']
...
for ind in range(len(train_keys)):
    ...
    if ind == 1: 
        info_list1 = [np.around(float(re.findall(r"-?\d+\.\d*", re.findall(r"%s\W+\d+\.\d+" % (key_name1), x)[0])[0]), 2)/200 for x in loss_lines]
    else:
        info_list1 = [np.around(float(re.findall(r"-?\d+\.\d*", re.findall(r"%s\W+\d+\.\d+" % (key_name1), x)[0])[0]), 2) for x in loss_lines]
    ...
    max_y = max(max(info_list1, info_list2))
    min_y = min(min(info_list1, info_list2))
...
# plt.show()
plt.figure(1).savefig('./figs/{:}_exp_curve.jpg'.format(dataset + '_' + exp_names[:-4]))
print('Save the loss curve plot at {:}'.format('./figs/' + dataset + '_' + exp_names + '_exp_curve.jpg'))
