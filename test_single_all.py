...
from core.model_test_single import model
...
from dataset import data_loader_mov_single_test as data_loader
...
if __name__ == "__main__":
    ...
    for idx in range(258): # For FD004, there are 248 test trajectories; FD002: 258
        ...
        data_iter = data_loader.CMPDataIter(config.data.root, config.data.set, config.data.max_rul, config.data.seq_len, config.net.name, test_id)
        if len(data_iter) == 1:
            print(" test id:{:} only contains one case, so we skip to test.".format(test_id))
            continue
        ...
        if vis:
            import pickle
            res_pt = 'res/{:}_test_engine{:}.pkl'.format(config.data.set, test_id)
