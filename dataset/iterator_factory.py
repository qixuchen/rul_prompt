...
def get_dataiter(data_name, config, mod='normal'):
    ...
    if mod == 'normal':
        ...
        if iid == 'iid':
            ...
            data_iter = CMPDataIterFed(root, set, llm_name, max_rul, seq_len, net_name, n_user, sample_interval, iid)
        elif iid == 'non_iid':
            from .data_loader_fed_non_iid import CMPDataIterFedNonIID, worker_init_fn
            data_iter = CMPDataIterFedNonIID(root, set, llm_name, max_rul, seq_len, net_name, n_user, sample_interval, iid)
        elif iid == 'non_iid_v2':
            from .data_loader_fed_non_iid_v2 import CMPDataIterFedNonIIDV2, worker_init_fn
            data_iter = CMPDataIterFedNonIIDV2(root, set, llm_name, max_rul, seq_len, net_name, n_user, sample_interval, iid)
